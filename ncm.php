...
class NCM {
    public static function dump($filename, $dealwithid3 = true, $in_path = "./ncm/", $output_path = "./output/") {
        ...
        if (Hex::strToHex($h) == $head) { //Check head
            ...
            $metadata = openssl_decrypt($metadata, 'AES-128-ECB', $metakey, OPENSSL_RAW_DATA); //利用openssl aes解密,得到元数据json
            ...
            $file = $output_path.str_replace(".ncm",null,$filename). "." .$format;
            ...
            if ($dealwithid3) {
                ...
                $tagwriter = new getid3_writetags; //初始化tagwriter
                $tagwriter->filename = $file; //指定文件
                $tagwriter->tagformats = array('id3v2.3'); //指定写入数据模式id3v2.3
                if($metadata['format'] == "flac") $tagwriter->tagformats = array('metaflac'); //解决flac无法写入元数据问题
                ...
                if ($previoustag) $TagData = $previoustag; //继承先前的id3数据(顺便把备注给去了)
                $TagData['title'][0] = $metadata['musicName']; //歌曲名
                $TagData['artist'][0] = $metadata['artist'][0][0]; //艺术家/演唱者
                $TagData['album'][0] = $metadata['album']; //专辑名
                if($APICdata) {
                  $TagData['attached_picture'][0]['data'] = $APICdata; //封面图片
                  $TagData['attached_picture'][0]['description'] = 'cover'; //封面图片描述
                    if($metadata['format'] != "flac") { //非flac格式(mp3)写入mimetype 这两行不兼容metaflac
